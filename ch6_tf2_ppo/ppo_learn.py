...
class PPOagent(object):
    ...
    def __init__(self, env):
        ...
        # 환경 environment
        self.env = env
        # 상태변수 차원   dim of the state
        self.state_dim = env.observation_space.shape[0]
        # 행동 차원      dim of the action
        self.action_dim = env.action_space.shape[0]
        # 행동의 최대 크기    max action
        self.action_bound = env.action_space.high[0]
        # 표준편차의 최솟값과 최댓값 설정    max of std
        self.std_bound = [1e-2, 1.0]

        # 액터 신경망 및 크리틱 신경망 생성, actor and critic NN generation
        ...
        # 에피소드에서 얻은 총 보상값을 저장하기 위한 변수, variable to save the total reward
        self.save_epi_reward = []


    ## 로그-정책 확률밀도함수 계산, log-policy
    ...
    ## 액터 신경망으로 정책의 평균, 표준편차를 계산하고 행동 샘플링, action sampling using the Gaussian actor
    ...
    ## GAE와 시간차 타깃 계산, GAE and TD target
    ...
    ## 배치에 저장된 데이터 추출, obtain the sample from batch
    ...
    ## 액터 신경망 학습, actor NN training
    def actor_learn(self, log_old_policy_pdf, states, actions, gaes):

        with tf.GradientTape() as tape:
            # 현재 정책 확률밀도함수, current policy
            mu_a, std_a = self.actor(states, training=True)
            log_policy_pdf = self.log_pdf(mu_a, std_a, actions)

            # 현재와 이전 정책 비율, ratio between the old policy and the current policy
    ...
    ## 크리틱 신경망 학습, critric NN training
    ...
    def train(self, max_episode_num):
        ...
        # 에피소드마다 다음을 반복 repeat the following for each episode
        for ep in range(int(max_episode_num)):

            # 에피소드 초기화, init episode
            time, episode_reward, done = 0, 0, False
            # 환경 초기화 및 초기 상태 관측, init env and measure the init state
            ...
            while not done:
                ...
                # 환경 가시화
                #self.env.render()
                # 이전 정책의 평균, 표준편차를 계산하고 행동 샘플링, compute the mean and var of the old policy
                mu_old, std_old, action = self.get_policy_action(tf.convert_to_tensor([state], dtype=tf.float32))
                # 행동 범위 클리핑, clipping the action
                action = np.clip(action, -self.action_bound, self.action_bound)
                # 이전 정책의 로그 확률밀도함수 계산, compute the pdf policy of the old policy
                var_old = std_old ** 2
                log_old_policy_pdf = -0.5 * (action - mu_old) ** 2 / var_old - 0.5 * np.log(var_old * 2 * np.pi)
                log_old_policy_pdf = np.sum(log_old_policy_pdf)
                # 다음 상태, 보상 관측, measure the next state and reward
                next_state, reward, done, _ = self.env.step(action)
                # shape 변환, shape convert
                state = np.reshape(state, [1, self.state_dim])
                action = np.reshape(action, [1, self.action_dim])
                reward = np.reshape(reward, [1, 1])
                log_old_policy_pdf = np.reshape(log_old_policy_pdf, [1, 1])
                # 학습용 보상 설정, set the reward for training
                train_reward = (reward + 8) / 8
                # 배치에 저장, save the data in the batch
                batch_state.append(state)
                batch_action.append(action)
                batch_reward.append(train_reward)
                batch_log_old_policy_pdf.append(log_old_policy_pdf)

                # 배치가 채워질 때까지 학습하지 않고 저장만 계속, wait until filling the batch 
                if len(batch_state) < self.BATCH_SIZE:
                    # 상태 업데이트 update the state
                    state = next_state
                    episode_reward += reward[0]
                    time += 1
                    continue

                # 배치가 채워지면, 학습 진행, start training when the batch is full
                # 배치에서 데이터 추출, get the data from the batch
                states = self.unpack_batch(batch_state)
                actions = self.unpack_batch(batch_action)
                rewards = self.unpack_batch(batch_reward)
                log_old_policy_pdfs = self.unpack_batch(batch_log_old_policy_pdf)
                # 배치 비움, empty the batch
                batch_state, batch_action, batch_reward, = [], [], []
                batch_log_old_policy_pdf = []
                # GAE와 시간차 타깃 계산, compute the GAE and TD target 
                ...
                # 에포크만큼 반복 repeat number of the epoch 
                for _ in range(self.EPOCHS):
                    # 액터 신경망 업데이트, actor update
