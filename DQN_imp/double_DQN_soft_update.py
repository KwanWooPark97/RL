...
class DQNtrain:
    ...
    def update_target(self):
        for t, e in zip(self.dqn_target.trainable_variables, 
            self.dqn_model.trainable_variables): t.assign(t * (1 - self.tau) + e * self.tau)
    ...
    def train(self):
        ...
        with tf.GradientTape() as tape:
            ...
            for i in range(self.batch_size):
                best_action = np.argmax(target_action[i])
                next_v = target_val[i, best_action]
                if dones[i]:
